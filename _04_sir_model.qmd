...
The transmission term is defined through the [force of infection (FOI), $\lambda$]{style="color:tomato;"}
...
- The frequency-dependent/mass action FOI is given by [$\lambda = \beta \times \dfrac{I}{N}$]{style="color:blue"}, where [$\beta$]{style="color:blue"} is the transmission rate.
- Assumes transmission does not increase with the density of infected individuals, [$\dfrac{I}{N}$]{style="color:blue"}.
...
- The density-dependent transmission is given as [$\lambda = \beta \times I$]{style="color:blue"}.

- Assumes transmission increases with the density of infected individuals, [$\dfrac{I}{N}$]{style="color:blue"}.
...
::: {.callout-note collapse="true" icon="false"}
- Compartments can be represented as [population sizes]{style="color:tomato;"} or [proportions]{style="color:tomato;"}
:::
...
::: callout-note
- For this introductory course, we will focus on simulation.
:::

- But first, let's do some qualitative analysis of the SIR model.
...
- Since at $t=0$$, I > 0$, we have [$\beta S - \gamma < 0$]{style="color:blue;"}, and [$S < \dfrac{\gamma}{\beta}$]{style="color:blue;"}.
...
- It is often represented as a single number or a range of high-low values.
  - For example, the $R0$ for measles is popularly known to be $12$ - $18$.
...
- $R0$ is [unitless and dimensionless]{style="color:tomato"}.
...
- This course focuses on the [R programming language]{style="color:tomato"} because:
...
- Let's do a code walk through in R using the Quarto script [`sir.qmd`](https://github.com/GWAC-MPPR/mppr_intro_to_idd_modelling_r_practicals/tree/main/model_implementation_practicals).
