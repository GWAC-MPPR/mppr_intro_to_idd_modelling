---
title: "Introduction to Infectious Disease Modelling"
subtitle: "Modelling for Pandemic Preparedness and Response Modular Shortcourse, 2026"
author:
  - name: "James Mba Azam, PhD"
    orcid: 0000-0001-5782-7330
    email: james.azam@lshtm.ac.uk
    affiliation: 
      - name: London School of Hygiene and Tropical Medicine, UK
        city: London, United Kingdom
date: "2026-09-10"
license: "CC BY"
copyright: 
  holder: James Mba Azam
  year: 2026
execute:
  echo: true
  freeze: auto
format:
  revealjs:
    theme: [solarized]
    slide-number: true
    scrollable: true
    chalkboard: true
    toc: true
    toc-depth: 1
transition: fade
lightbox: true
progress: true
code-copy: true
bibliography: references.bib
link-citations: true
engine: knitr
editor_options: 
  chunk_output_type: console
---

## Schedule

::::: columns
::: {.column width="50%"}
### Day 1

- An overview of infectious diseases
- Infectious disease models
- Types of infectious disease models
- Introduction to compartmental models
  - The SIR model
  - The SEIR model
- R practicals: simulating the SIR and SEIR models
:::

::: {.column width="50%"}
### Day 2

- Modelling epidemic control
  - Vaccination
  - Non-pharmaceutical interventions
- Brief overview of modelling host heterogeneity
- Stochasticity in SIR models
- R practicals: control measures and stochastic simulation
- Final remarks
:::
:::::

# An overview of infectious diseases

{{< include _01_infectious_diseases_overview.qmd >}}

# Infectious disease models

{{< include _02_infectious_disease_models.qmd >}}

# Types of infectious disease models

{{< include _03_01_types_of_models.qmd >}}

# Introduction to compartmental models

{{< include _03_02_compartmental_models.qmd >}}

## The Susceptible-Infected-Recovered (SIR) model

{{< include _04_sir_model.qmd >}}

## The Susceptible-Exposed-Infected-Recovered (SEIR) Model

{{< include _05_seir_model.qmd >}}

# Modelling epidemic control

{{< include _06_controlling_epidemics.qmd >}}

# Brief overview of modelling host heterogeneity

{{< include _07_heterogeneities.qmd >}}

# Stochasticity in SIR models

{{< include _08_stochasticity.qmd >}}

------------------------------------------------------------------------

# Final Remarks

## Takeaways

{{< include _takeaways.qmd >}}

------------------------------------------------------------------------

## What skills are needed to build and use infectious disease models?

{{< include _modelling_skills.qmd >}}

------------------------------------------------------------------------

## Contact Information

{{< include _contact_info.qmd >}}

------------------------------------------------------------------------

![](https://i.creativecommons.org/l/by/4.0/88x31.png) This presentation is made available through a [Creative Commons Attribution 4.0 International License](https://creativecommons.org/licenses/by/4.0/)

# List of Resources

{{< include _resources.qmd >}}

# References

::: {#refs}
:::
