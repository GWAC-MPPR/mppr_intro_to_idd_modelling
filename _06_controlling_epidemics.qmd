...
- Modify the `sir.qmd` script to simulate the SEIRV model.

- Download the file `Day 2 R practicals pack` from Moodle or clone it from GitHub using the following command in your terminal
...
- Conceptually, NPIs usually act to either [reduce the transmission rate]{style="color:tomato"}, $\beta$ or prevent infected individuals from transmitting.

- NPIs like quarantine, social distancing and movement restrictions can reduce the transmission rate, $\beta$, by [reducing the contact rate]{style="color:tomato"} between susceptible and infectious individuals.

- Hygiene measures reduce the [probability of transmission]{style="color:tomato"} per contact, thereby reducing the transmission rate, $\beta$, since $\beta = c \times p$.
...
- The simplest form is to reduce $\beta$ by an [NPI efficacy]{style="color:blue"}, say $\epsilon$.

- Assuming the NPI is implemented between [$t_{\text{npi\_start}}$]{style="color:blue"} and [$t_{\text{npi\_end}}$]{style="color:blue"}, it means that $\beta$ remains the same before that period and is modified to [$(1- \epsilon)\beta$]{style="color:blue"} during the period of the NPI, where [$0 \leq \epsilon \leq 1$]{style="color:blue"}.
...
- Let's open the script file `sir_npi.qmd` and follow along.
- Download the file `Day 2 R practicals pack` from Moodle or clone it from GitHub using the following command in your terminal
...
- This means that infected individuals in isolation [do not contribute to the transmission]{style="color:tomato"} of the disease and need to be removed from the infected compartment.

- Moving isolated individuals to a separate compartment allows us to [track them separately]{style="color:tomato"} in the model and apply relevant parameters.
...
- Sketch the model diagram for the SIR model with an isolated compartment, $Q$. Which arrows leave $I$, and where do individuals go when they leave $Q$?
- Now write down the model equations. Assume isolated individuals recover at a rate $\tau$.
:::

------------------------------------------------------------------------

The modified SIR model with isolation is as follows:

::::: columns
::: {.column width="50%"}
::: {.content-visible unless-format="pdf"}
![](images/siqr_model.svg){width="100%"}
:::

::: {.content-visible when-format="pdf"}
![](images/siqr_model.pdf){width="100%"}
:::
:::
...
- Modify the SIR model in [`sir.qmd`](https://github.com/GWAC-MPPR/mppr_intro_to_idd_modelling_r_practicals/tree/main/model_implementation_practicals) to incorporate the isolated compartment, $Q$ and the relevant parameters.
- Download the file `Day 2 R practicals pack` from Moodle or clone it from GitHub using the following command in your terminal
